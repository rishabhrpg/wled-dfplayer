<p align="center">
  ...
  # Latest: DFPlayer usermod (2025-09-09)

  - Commit: `0c3030f2` — "add. dfplayer usermod"

  This update adds a DFPlayer MP3 module usermod under `usermods/dfplayer`.

  Quick notes:
  - Enable the mod by adding it to the environment's `custom_usermods` in `platformio.ini`, e.g.

  ```ini
  custom_usermods = audioreactive, dfplayer
  ```

  - The usermod uses Serial logging (e.g. `Serial.println()`), so open the serial monitor at 115200 baud to see logs:

  ```sh
  pio device monitor --baud 115200
  ```

  - After enabling, rebuild the web UI and firmware:
